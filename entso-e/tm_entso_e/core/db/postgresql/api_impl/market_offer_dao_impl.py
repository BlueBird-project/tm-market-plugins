...
class MarketOfferQueries:
    GET_MARKET_OFFER_DETAILS = """SELECT  "offer_id",  "market_id", "sequence", "ts_start", "ts_end", "isp_unit",  
      "update_ts", "ext" FROM "${table_prefix}market_offer_details" WHERE offer_id = :offer_id """

    LIST_MARKET_OFFER_DETAILS = """SELECT offer_details."offer_id", offer_details."market_id",
    offer_details."sequence", offer_details."ts_start", offer_details."ts_end", offer_details."isp_unit",  
    offer_details."update_ts", offer_details."ext" FROM "${table_prefix}market_offer_details" as offer_details
    ...
    SELECT_MARKET_OFFER_BY_ID = """SELECT "ts","offer_id", "isp_start","isp_len", "cost_mwh"  ,"update_ts" 
    ...
    ("market_id", "sequence", "ts_start", "ts_end", "isp_unit",  "update_ts", "ext")
    VALUES (:market_id, :sequence, :ts_start, :ts_end, :isp_unit,   extract(epoch from now()) * 1000, :ext)   """
    #     TODO: on conflict

    INSERT_MARKET_OFFER = """  INSERT INTO "${table_prefix}market_offer_details" 
    ("ts","offer_id", "isp_start","isp_len", "cost_mwh"  ,"update_ts" )
    VALUES (:ts, :offer_id, :isp_start, :isp_len, :cost_mwh, extract(epoch from now()) * 1000 )   """
...
class MarketOfferDAOImpl(MarketOfferDAO):
    ...
    def get_offer_details(self, offer_id: int) -> Optional[MarketOfferDetails]:
        with ConnectionWrapper() as conn:
            return conn.get(q=self.queries.LIST_MARKET_OFFER_DETAILS, args={"offer_id": offer_id},
