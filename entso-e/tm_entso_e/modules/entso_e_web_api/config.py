...
class ENTSOEServiceSettings(DictBaseSettings):
    ...
    @classmethod
    def load(cls, yaml_path: Optional[str] = None, **kwargs):
        # regex = re.compile(r'^__?.+_?_?$')
        if yaml_path:
            app_config = load_yaml_obj(yaml_path, section=__CONFIG_SECTION__.lower(), settings_constructor=dict)
        ...
        # return super().load(yaml_path=yaml_path, section_name="KE".lower
...
def configure_api() -> ENTSOEAPISettings:
    ...
    if app_args.config_path is not None:
        service_settings = ENTSOEServiceSettings.load(yaml_path=app_args.config_path)
    ...
    _api_settings = load_yaml_obj(api_config_path, section=ENTSOEAPISettings.__SECTION__, settings_constructor=dict)
